...
namespace meshy {
    ...
    {
        LOG(LOG_DEBUG) << "Server ready, wait for new connection ..." ;
    ...
    void IOCPLoop::_IOCPConnectionThread(IOCPServer* server)
    {
        while (!_shutdown)
        {
            LOG(LOG_DEBUG) << "to accept" ;
            WSAConnectionPtr connection = server->Accept(static_cast<int32_t>(server->GetNativeSocket()));
    ...
    void IOCPLoop::_WorkThread(int32_t listenfd, HANDLE completionPort)
    {
        ...
        while (!_shutdown) {
            ...
            if (0 == result) {
                LOG(LOG_ERROR) << "GetQueuedCompletionStatus Error: " << GetLastError() ;
    ...
    void IOCPLoop::_Enqueue(IOCPStreamPtr stream, const char* buf, int64_t nread)
    {
        ...
        if (stream->GetDataSink()) {
            LOG(LOG_DEBUG) << "_Enqueue, datasink registered." ;
        }
    }

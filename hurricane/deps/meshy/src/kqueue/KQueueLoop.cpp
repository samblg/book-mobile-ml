...
KQueueLoop::KQueueLoop(const std::string& host, int32_t port) :
...
void KQueueLoop::ServerListenThread()
{
    ...
    if (-1 == sock)
    {
        LOG(LOG_ERROR) << "socket() failed:" << errno ;
        return;
    }
    ...
    {
        LOG(LOG_ERROR) << "bind() failed:" << errno ;
        return;
    }

    if (listen(sock, 5) == -1)
    {
        LOG(LOG_ERROR) << "listen() failed:" << errno ;
        return;
    }
    ...
    if (0 != registrar)
    {
        LOG(LOG_ERROR) << "Register listener to kqueue failed.\n";
        return;
    }
    ...
    while (true)
    {
        int32_t ret = kevent(kq, NULL, 0, events, MAX_EVENT_COUNT, NULL);
        if (ret == -1)
        {
...
int32_t KQueueLoop::_RegisterKQueue(int32_t kq, int32_t fd)
{
    ...
    int32_t ret = kevent(kq, changes, 1, NULL, 0, NULL);
    if (-1 == ret)
        return -1;
...
void KQueueLoop::_Accept(int32_t kq, int32_t fd, int32_t connSize)
{
    ...
    {
        int32_t client = accept(fd, NULL, NULL);
        if (client == -1)
        {
        ...
        {
            LOG(LOG_ERROR) << "Register client failed.\n";
            return;
        }
    }
}
...
void KQueueLoop::_Read(int32_t sock, uint32_t availBytes)
{
    ...
    if (bytes == 0 || bytes == -1)
    {
        ...
        LOG(LOG_ERROR) << "client Close or recv failed.\n";
        return;
    }
